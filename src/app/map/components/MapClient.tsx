...
export default function MapClient() {
  ...
  const generateProjectsMutation = api.roadmap.generateProjects.useMutation({
    onSuccess: (data) => {
      console.log("🚀 Projects generated successfully:", data.data);
  ...
  const saveProjectsMutation = api.roadmap.saveProjects.useMutation({
    onSuccess: (data) => {
      console.log("💾 Projects saved to database:", data.data);
  ...
  useEffect(() => {
    if (getProjectsQuery.data?.success && getProjectsQuery.data.data) {
      console.log("📋 Projects loaded from database:", getProjectsQuery.data.data);
                                  ...
                                  <h4 
                                    ...
                                    onClick={async () => {
                                      ...
                                      if (!existingTopics) {
                                        ...
                                        console.log("🎯 Extracting topics for YouTube video before navigation...");
                                          ...
                                          if (response.ok) {
                                            ...
                                            try {
                                              if (result.result?.data?.json?.data?.topics) {
                                                ...
                                                console.log("✅ Found topics in structure: result.result.data.json.data.topics");
                                              } else if (result.result?.data?.success && result.result.data.data?.topics) {
                                                topics = result.result.data.data.topics;
                                                console.log("✅ Found topics in structure: result.result.data.data.topics");
                                              } else if (result.result?.data?.topics) {
                                                topics = result.result.data.topics;
                                                console.log("✅ Found topics in structure: result.result.data.topics");
                                            ...
                                            if (topics && Array.isArray(topics)) {
                                              ...
                                              console.log("✅ Topics extracted and saved to localStorage before navigation:", topicsData.topics.length, "topics");
                                              console.log("🔑 localStorage key used:", `topics_${contentId}`);
                                              console.log("📝 Topics data saved:", JSON.stringify(topicsData, null, 2));
                                              
                                              // Verify the data was saved correctly
                                              const verifyData = localStorage.getItem(`topics_${contentId}`);
                                              console.log("🔍 Verification - data in localStorage:", verifyData ? "Found" : "Not found");
                                              ...
                                              console.warn("⚠️ Topic extraction failed, proceeding without topics");
                                              // Navigate anyway
                                              window.open(`/learn/${contentId}`, '_blank');
                                            }
                                          } else {
                                            console.warn("⚠️ API call failed, proceeding without topics");
                                            // Navigate anyway
                                            window.open(`/learn/${contentId}`, '_blank');
                                          }
                                        } catch (error) {
                                          console.warn("⚠️ Failed to extract topics, proceeding without them:", error);
                                        ...
                                        console.log("📚 Topics already exist for this video, using cached version");
